...
/* Refer to MS-OXRTFCP for details */
...
static PyMethodDef xrtf_functions[] = {
...
{
    PyModule_AddFunctions(m, xrtf_functions);

    if (PyStructSequence_InitType2(&CompressedRtfHeader_Type, &CompressedRtfHeader_desc) == -1)
        goto fail;
    PyModule_AddObject(m, "CompressedRtfHeader", (PyObject*) &CompressedRtfHeader_Type);
